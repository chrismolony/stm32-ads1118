...
//数据入列
uint32_t remote_data_x_usart_In_Queue(uint8_t ch) 
{ 	
	if(USARTm_Tx_Buf_Struct.USARTm_Tx_COUNTER <USARTm_Tx_BUFFER_SIZE)
		{
			if(USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_HEAD >= USARTm_Tx_BUFFER_SIZE) 
				USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_HEAD = 0;  		
			USARTm_Tx_Buf_Struct.USARTm_Tx_Buffer[USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_HEAD] = ch; 		
			USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_HEAD++; 		
			USARTm_Tx_Buf_Struct.USARTm_Tx_COUNTER++;  		
			return 0; 	
		} 		
	return 1;	 	 
}
//数据出列
uint32_t remote_data_x_usart_Out_Queue(uint8_t *ch) 
{ 	
	uint32_t num;
	if(USARTm_Tx_Buf_Struct.USARTm_Tx_COUNTER > 0) 	
		{ 		
			if(USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_TAIL >= USARTm_Tx_BUFFER_SIZE) 			
				USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_TAIL = 0;
			num = USARTm_Tx_Buf_Struct.USARTm_Tx_COUNTER; 		
			*ch = USARTm_Tx_Buf_Struct.USARTm_Tx_Buffer[USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_TAIL]; 		
			USARTm_Tx_Buf_Struct.USARTm_Tx_PTR_TAIL++; 		
			USARTm_Tx_Buf_Struct.USARTm_Tx_COUNTER--;  		
			return num; 	
		} 	
	else 	
...
float_union temp;
uint8_t USARTmTxBuffer[USARTm_Tx_BUFFER_SIZE];
uint8_t USARTmRxBuffer[USARTm_Rx_BUFFER_SIZE];
uint8_t USARTmRxBufferD[USARTm_Rx_BUFFER_SIZE];
...
void remote_data_x_usart_gpio_config(void)
{
  ...
  GPIO_Init(USARTm_GPIO, &GPIO_InitStructure);
	...
	GPIO_InitStructure.GPIO_Pin = RS485_ENABLE_PIN;//RS485使能
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init(RS485_ENABLE_GPIO, &GPIO_InitStructure);

}
//485读使能
void remote_data_x_usart_read_enable(void)
{
	GPIO_ResetBits(RS485_ENABLE_GPIO,RS485_ENABLE_PIN);
}
//485写使能
void remote_data_x_usart_write_enable(void)
{
	GPIO_SetBits(RS485_ENABLE_GPIO,RS485_ENABLE_PIN);
}
//NVIC配置
...
//DMA-发送通道配置
void remote_data_x_usart_dma_tx_config(uint32_t memoryBaseAddr, uint8_t sendBufferSize)
{
...
//DMA-接收通道配置
void remote_data_x_usart_dma_rx_config(void)	
{
	DMA_InitTypeDef DMA_InitStructure;
...
//测试用，写一个浮点
void remote_data_x_usart_dma_write(unsigned char cmd,float ad)//
{
		switch(cmd){
			
			case ADS1118_VALUE:
				temp.fv = ad;
			  memset(USARTmTxBuffer,0,sizeof(USARTmTxBuffer));
				USARTmTxBuffer[0] = temp.cv[0];
				USARTmTxBuffer[1] = temp.cv[1];
				USARTmTxBuffer[2] = temp.cv[2];
				USARTmTxBuffer[3] = temp.cv[3];
				USARTmTxBuffer[4] = 0x00;
				USARTmTxBuffer[5] = 0x00;
				USARTmTxBuffer[6] = 0x00;
				USARTmTxBuffer[7] = 0x00;
				USARTmTxBuffer[8] = 0x00;
				USARTmTxBuffer[9] = 0x00;
				USARTmTxBuffer[10] = 0x00;
				USARTmTxBuffer[11] = 0x00;
				USARTmTxBuffer[12] = 0x00;
				USARTmTxBuffer[13] = 0x00;
				USARTmTxBuffer[14] = 0x00;
				USARTmTxBuffer[15] = 0x00;
				USARTmTxBuffer[16] = 0x00;
				USARTmTxBuffer[17] = 0x00;
				USARTmTxBuffer[18] = 0x00;
				USARTmTxBuffer[19] = 0x00;
				USARTmTxBuffer[20] = 0x00;
				remote_data_x_usart_dma_start_tx((uint32_t)USARTmTxBuffer,21);
				break;
...
void remote_data_x_usart_dma_start_tx(uint32_t memoryBaseAddr,uint32_t SendBufferSize)
{
    remote_data_x_usart_dma_tx_config(memoryBaseAddr,SendBufferSize);
...
void USART1_IRQHandler(void)//中断向量定义在startup_stm32f10x.s中
{
    ...
    {
        remote_data_x_usart_dma_read();
				USART_ReceiveData( USARTm );
    }
}
//DMA读出一帧数据，开始下一帧的等待接收
void remote_data_x_usart_dma_read(void)
{
    uint8_t rxcounter;
		uint8_t i;
    ...
    memset(USARTmRxBufferD,0,sizeof(USARTmRxBufferD));
	  for(i=0;i<rxcounter;i++){
		...
		//remote_data_x_get_data(USARTmRxBufferD);//分析收到的数据包
		for(i=0;i<rxcounter;i++)
...
//外部调用函数 初始化
void remote_data_x_usart_Init(void)
{
	
	remote_data_x_usart_rcc_config();
	remote_data_x_uasrt_nvic_config();
	remote_data_x_usart_gpio_config();
	
	remote_data_x_usart_dma_rx_config();//接收配置
	remote_data_x_usart_config();
}
...
{ 	
	...
	if(DMA_GetCurrDataCounter(USARTm_Tx_DMA_Channe)==0) 
	{
		...
		if(num>0) 		
		{ 			
			remote_data_x_usart_dma_start_tx((uint32_t)USARTmTxBuffer,num);
		} 		
		return 0; 	
	} 	
	else 	
...
PUTCHAR_PROTOTYPE
{
 ...
 // while (USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET);
	remote_data_x_usart_In_Queue((uint8_t)ch);//将要发送的数据放入队列
  return ch;
}
