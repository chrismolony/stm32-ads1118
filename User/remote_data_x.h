...
void remote_data_x_usart_dma_rx_config(void);//DMA接收配置
void remote_data_x_usart_dma_tx_config(uint32_t memoryBaseAddr, uint8_t sendBufferSize);//DMA发送配置 
void remote_data_x_usart_dma_read(void);//DMA 读串口数据 在串口的空闲中断中调用，或者重写该函数，放到其他地方调用
void remote_data_x_usart_rcc_config(void);

//****************外部调用函数************************
void remote_data_x_usart_Init(void);//初始化
void remote_data_x_usart_dma_write(unsigned char cmd,float ad);//DMA 写串口数据
void remote_data_x_usart_read_enable(void);//读485数据使能
void remote_data_x_usart_write_enable(void);//写485数据使能
void remote_data_x_usart_dma_start_tx(uint32_t memoryBaseAddr,uint32_t SendBufferSize);//使用DMA发送时给出发送缓存首地址和长度即可
int remote_data_x_usart_dma_ctl(void);//该函数用于printf的实现，在串口输出打印调试信息
uint32_t remote_data_x_usart_In_Queue(uint8_t ch);//将数据放入队列
uint32_t remote_data_x_usart_Out_Queue(uint8_t *ch);//将数据读出队列
#endif
//@StevenShi 2017/09/05
/**************************************************************************************************/
